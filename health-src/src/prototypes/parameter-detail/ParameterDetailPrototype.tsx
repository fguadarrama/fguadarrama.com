...
import ComparisonClinical from './ComparisonClinical'
import TrendCanvas from './TrendCanvas'
import SideInspector from './SideInspector'

const variants = [
  { name: 'Comparación clínica', axis: 'gráfica y tabla simultáneas' },
  { name: 'Lienzo de tendencia', axis: 'evolución dominante' },
  { name: 'Inspector lateral', axis: 'contexto persistente' },
]
...
function Picker({ current, setCurrent, replay }: { current: number; setCurrent: (index: number) => void; replay: () => void }) {
  ...
  return <nav ref={pickerRef} className="proto-picker" aria-label="Prototype variants">
...
export default function ParameterDetailPrototype() {
  ...
  const surface = useMemo(() => {
    if (current === 0) return <ComparisonClinical />
    if (current === 1) return <TrendCanvas />
    return <SideInspector />
  }, [current, replayKey])
